...
| **tpt-eng-electrical** | Electrical primitives: impedance/reactance, per-unit systems, three-phase power, conductor/insulator property lookups | 329 | Complex impedance/reactance (R/L/C series/parallel combination), per-unit conversions, balanced three-phase power, conductor/insulator lookup table with `DataSource` provenance; depends on `tpt-math-units`, `tpt-math-numeric` |
| **tpt-eng-heat-transfer** | Heat-transfer correlations: convection (Nu/Re/Pr), 1D/radial conduction, radiation view factors, thermal-resistance networks | 233 | Nusselt correlations (flat plate / cylinder / sphere / internal pipe, laminar + turbulent), 1D + radial conduction, grey-body radiation exchange, series/parallel `R_th` composition; uses `tpt-eng-props-air`/`-water` |
| **tpt-eng-props-mixture** | General real-gas / VLE property lookups for arbitrary process mixtures (`no_std`); joins the `tpt-eng-props` umbrella | 540 | Peng–Robinson EOS (compressibility-factor `Z` roots, component fugacity coefficients, bubble/dew-point pressure) with vdW one-fluid mixing; `no_std` via `tpt-math-numeric::libm`; re-exported by `tpt-eng-props` |
| **tpt-eng-pcb** | PCB layer stackup, trace routing primitives, via/footprint definitions | 326 | Microstrip characteristic impedance, IPC-2221 trace current capacity, dielectric layer stackup, via/pad footprints; depends on `tpt-eng-electrical`, `tpt-eng-materials` |
| **tpt-eng-thermal-mgmt** | Heat sink sizing, fan curves, thermal-resistance networks for electronics | 213 | Straight-fin efficiency, extruded heatsink spreading/contact `R_th`, fan-curve operating-point intersection, junction-to-ambient networks; depends on `tpt-eng-heat-transfer`, `tpt-eng-props-air` |
| **tpt-eng-power-components** | Transformer/generator equivalent circuits, transmission-line parameters | 650 | Two-winding transformer equivalent circuit (leakage + magnetizing), induction/synchronous generator slip power split, long-line `ABCD` two-port parameters; depends on `tpt-eng-electrical` |
| **tpt-eng-renewables** | Solar PV, wind turbine, battery degradation models | 341 | Single-diode PV I–V/P–V, Betz-limited wind power curve, Li-ion Weibull capacity-fade (on `tpt-eng-reliability`); depends on `tpt-eng-electrical`, `tpt-eng-props-air`, `tpt-eng-reliability` |
| **tpt-eng-vehicle-dynamics** | Tire models, aero drag, suspension kinematics | 336 | Pacejka "magic formula" tire forces, quadratic aero drag/lift, double-wishbone roll-center kinematics; depends on `tpt-eng-geometry`, `tpt-eng-structural`, `tpt-math-linalg` |
| **tpt-eng-biomech** | Hyperelastic tissue models, implant geometry | 299 | Mooney-Rivlin / Ogden strain-energy stress evaluation, implant stem/cup parametric geometry (on `tpt-eng-geometry`); depends on `tpt-eng-materials`, `tpt-eng-geometry` |
| **tpt-eng-unit-ops** | Distillation, heat exchangers, pump/compressor curves | 838 | McCabe–Thiele distillation (Fenske/Underwood/Gilliland), ε-NTU + LMTD exchangers, pump/compressor curves & NPSH; depends on `tpt-eng-props`, `tpt-eng-heat-transfer` |
| **tpt-eng-crystallography** | Miller indices, slip systems, crystal symmetry | 418 | Miller-index plane/direction vectors, FCC/BCC/HCP slip systems, cubic point-group rotation matrices (`f32`, over `tpt-eng-geometry`); depends on `tpt-eng-geometry` |
| **tpt-eng-geotech** | Soil constitutive models, borehole stratigraphy | 512 | Mohr-Coulomb failure envelope + FoS, modified Cam-Clay yield/void-ratio update, Terzaghi/Meyerhof bearing capacity, 1-D consolidation time-rate, Rankine/Coulomb lateral pressure, Atterberg/USCS classification, borehole stratigraphy with `DataSource`; depends on `tpt-eng-materials` |
| **tpt-eng-building-sys** | HVAC loads, plumbing fixture units, electrical panel scheduling | 727 | HVAC envelope/infiltration/internal-gain loads, Hunter fixture-unit demand, three-phase panel scheduling; depends on `tpt-eng-props-air`, `tpt-eng-heat-transfer`, `tpt-eng-electrical` |
| **tpt-eng-schedule** | CPM/PERT networks, resource leveling, Earned Value Management | 602 | CPM/PERT activity-on-node forward/backward pass + float, PERT variance, resource leveling, Earned Value metrics; depends only on `tpt-math-numeric` |
